...
def test_is_narrower():
    ...
    assert is_narrower(Annotation(int), Annotation(Any))
    assert is_narrower(Annotation(int), Any)
    assert is_narrower(int, Annotation(Any))
    assert is_narrower(int, Any)

    # Any is BOTH top and bottom type - bidirectional relationships
    assert is_narrower(Any, int)  # bottom type behavior
    assert is_narrower(Any, object)  # bottom type behavior
    assert is_narrower(Annotation(Any), Annotation(int))  # bottom type
    assert is_narrower(Any, Any)  # top and bottom meet

    # verify with alias
    assert is_narrower(SimpleAlias, UnionAlias)
    assert is_narrower(ListAlias, AnnotatedListAlias)

    # verify with Any in generics - bidirectional
    assert is_narrower(list[int], list[Any])  # top type
    assert is_narrower(list[Any], list[int])  # bottom type


def test_is_narrower_generics():
    # direct ABC comparisons
    assert is_narrower(list[int], Sequence[int])

    assert is_narrower(list[int], Sequence[int])
    assert is_narrower(dict[str, int], Mapping[str, int])
    ...
    assert is_narrower(MyList, list[int])
    assert is_narrower(MyList, list[Any])
    assert is_narrower(MyList, list)
    assert not is_narrower(MyList, list[str])

    assert is_narrower(MyListSubclass, list[int])
    assert is_narrower(MyListSubclass, list[Any])
    assert is_narrower(MyListSubclass, list)
    assert not is_narrower(MyListSubclass, list[str])

    # subclass to ABC comparisons
    assert is_narrower(MyList, Sequence[int])
    assert not is_narrower(MyList, Sequence[str])

    assert is_narrower(MyListSubclass, Sequence[int])
    assert not is_narrower(MyListSubclass, Sequence[str])

    # nested generics with ABCs
    assert is_narrower(list[list[int]], Sequence[list[int]])
    assert is_narrower(list[list[int]], Sequence[Sequence[int]])

    class MyListOfLists(list[list[int]]):
        pass

    assert is_narrower(MyListOfLists, Sequence[list[int]])
    assert is_narrower(MyListOfLists, Sequence[Sequence[int]])


def test_is_narrower_protocols():
    # based on method
    @runtime_checkable
    ...
    assert is_narrower(Concrete, TestProtocol)
    ...
    assert is_narrower(ConcreteAttr, TestAttrProtocol)
    assert is_narrower(ConcreteAttr2, TestAttrProtocol)
